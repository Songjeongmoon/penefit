			...
			<div class="content" id="listBox">
				...
				<div class="pageBox">
					<c:if test="${start ne 1 }">
						<a
							href="/class/classList-expired?pageNum=${start - 5}&start=${start - 5}">[이전]</a>
					</c:if>
					<c:forEach var="pageNum" begin="${start }" end="${end }">
						<c:if test="${pageNum lt count  }">
							<a
								href="/class/classList-expired?pageNum=${pageNum }&start=${start}">${pageNum }</a>&nbsp;&nbsp;
              		</c:if>
					</c:forEach>
					<c:if test="${start + 5 lt count }">
						<a
							href="/class/classList-expired?pageNum=${start + 5}&start=${start + 5}">[다음]</a>
					</c:if>
				</div>
