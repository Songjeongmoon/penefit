         ...
         <div class="content" id="listBox">
            ...
            <div class="pageBox">
               <c:if test="${startNum ne 1 }">
                  <a
                     href="/class/classList-search?pageNum=${startNum - 1}&startNum=${startNum - 1}&keyword=${keyword}">[이전]</a>
               </c:if>
               <c:forEach var="pageNum" begin="${startNum }"
                  end="${startNum + 4 }">
                  <c:if test="${pageNum le result  }">
                     <a href="/class/classList-search?pageNum=${pageNum }&keyword=${keyword}">${pageNum }</a>&nbsp;&nbsp;
                        </c:if>
               </c:forEach>
               <c:if test="${startNum + 5 le result }">
                  <a
                     href="/class/classList-search?pageNum=${startNum + 5}&startNum=${startNum + 5}&keyword=${keyword}">[다음]</a>
               </c:if>
            </div>
         </div>
      </section>
