...
<body oncontextmenu="return false" onselectstart="return false" ondragstart="return false">
		...
		<div style="width: 100%;">
						...
						<tbody>
							<c:forEach var="suggestion" items="${suggestionList }">
								<tr>
									...
									<td><c:choose>
											<c:when test="${fn:length(suggestion.suggest_title) > 10}">
												<c:out value="${fn:substring(suggestion.suggest_title,0,10)}" />....
          									 </c:when>
											<c:otherwise>
							...
							<tbody>
								<c:forEach var="vo" items="${classTodayList }">
									<tr>
										<td>
											<c:choose>
											<c:when test="${fn:length(vo.class_subject) > 10}">
												<c:out value="${fn:substring(vo.class_subject,0,10)}" />....
          									 </c:when>
											<c:otherwise>
			...
			<div id="suggestionArticle" class="article">
				...
				<div class="box" id="suggestbox3">
					...
					<div class="content">
						<input type="text" name="suggestKeyword" id="suggestSearchBar" style="margin: 10px;"> <input type="button" value="검색" id="suggestSearchbtn"> <br> &nbsp;&nbsp;<a class="filterKeyword">검토</a>&nbsp;&nbsp;| <a class="filterKeyword">승인</a>&nbsp;&nbsp;| <a class="filterKeyword">반려</a>&nbsp;&nbsp;| <a onclick="suggestAll()">전체목록</a>
	...
	<script>
        ...
        $(".filterKeyword").click(function(evt){
        	...
        	alert(keyword);
        	getListWithFilter(keyword);
        })
        function getListWithFilter(keyword) {
          $("#suggestAllTbody").empty();
          $.ajax({
            method: "GET",
            url: "/admin/getListWithFilter",
            datatype: "json",
            data: {
              "keyword": keyword
            },
